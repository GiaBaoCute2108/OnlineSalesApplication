...
public class DienThoaiAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        if (viewType == VIEW_TYPE_DATA) {
            View v = LayoutInflater.from(context).inflate(R.layout.item_dienthoai, parent, false);
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof MyViewHolder) {
            ...
            if (sanPhamMoi != null) {
                Glide.with(context).load(sanPhamMoi.getHinhanh()).into(viewHolder.imghinhanh);
                viewHolder.tensp.setText(sanPhamMoi.getTensp());
                DecimalFormat decimalFormat = new DecimalFormat("###,###,###");
                viewHolder.giasp.setText("Giá: " + decimalFormat.format(Double.parseDouble(sanPhamMoi.getGia()))+"đ");
                viewHolder.mota.setText("Mô tả: " + sanPhamMoi.getMota());
                viewHolder.id.setText("Id: " + sanPhamMoi.getId());
